...
int main(int argc, char* argv[])
{
    ...
    if(argc != 5)
    {
        ...
        cerr << "Enter:\t arg_1 = hyperneat_config\t arg_2 =  user_definition_file \t arg_3 = genetic_encoding_file\t arg_4 = port_number" << endl;
        return -1;
    }
	...
	RobotVREP * vrep = new RobotVREP(false, atoi(argv[4]));   
    ...
    //cout << "NX*NY+ADITIONAL_HYPERNEAT_INPUTS = " << NX*NY+ADITIONAL_HYPERNEAT_INPUTS << endl;
    ...
    Population * cppn_neat;                     // CPPN for HyperNEAT

    // Creates a cppn for HyperNEAT
	...
	for(int g = 0; g < cppn_neat->GENERATIONS; g++)
	{
		...
		for(int p = 0; p < cppn_neat->POPULATION_MAX; p++)
		{
            ...
            bool flag = true;
            int flag_times = 0;
            double rightVel = 0.0;                  // right wheel velocity
            double leftVel = 0.0;                   // left wheel velocity
            double L_ANN = 0;
            double R_ANN = 0;
            double inverse_v = 0;
            ...
            hyperneat->createSubstrateConnections( &cppn_neat->organisms.at( p ));
			...
			{            
                ...
                *input.at(NX*NY+2) = (t_recoil>0) ? 1 : 0;   // Colisions
                ...
                /*diff_v += *output.at(0);
                speed_up += *output.at(1);                

                // Anti wind-up differential velocity filter
                if(diff_v >= 0) diff_v = (diff_v > MAX_SPEED_DIF + speed_up/2) ? MAX_SPEED_DIF + speed_up/2 : diff_v;                
                else diff_v = (-diff_v > MAX_SPEED_DIF + speed_up/2) ? -MAX_SPEED_DIF - speed_up/2 : diff_v;    
                //if(diff_v >= 0) diff_v = (diff_v > MAX_SPEED_DIF) ? MAX_SPEED_DIF : diff_v;                   
                //else diff_v = (-diff_v > MAX_SPEED_DIF) ? -MAX_SPEED_DIF : diff_v;    

                // Anti wind-up absolute velocity filter
                if(speed_up >= 0) speed_up = (speed_up > MAX_VEL - DEFAULT_SPEED) ? MAX_VEL - DEFAULT_SPEED : speed_up;
                else speed_up = (-speed_up > MIN_VEL + DEFAULT_SPEED) ? -MIN_VEL - DEFAULT_SPEED : speed_up;

                if ( ( (int) sim_time + bonus_tc + bonus_tz - discount_t) % TURNING_TIME == 0)
                {
                    diff_v = 0; // since I want to control direction and not speed
                }

                rightVel = DEFAULT_SPEED + floor(speed_up + diff_v/2);
                leftVel = DEFAULT_SPEED + floor(speed_up - diff_v/2);


                if(leftVel - rightVel > MAX_SPEED_DIF)
                {
                    rightVel = -MAX_SPEED_DIF/2;
                    leftVel = MAX_SPEED_DIF/2; 
                }

                else if (rightVel - leftVel > MAX_SPEED_DIF)
                {
                    rightVel = MAX_SPEED_DIF/2;
                    leftVel = -MAX_SPEED_DIF/2; 
                }                         

                cout << "output(0): " << *output.at(0) << " output(1): " << *output.at(1) << " diff_v: " << diff_v << " speed_up: " << speed_up << " leftVel: " << leftVel << " rightVel: " << rightVel << endl;
                */                                         
            ...
            cppn_neat->organisms.at(p).fitness = fitness->calculateFitness(n_colision, y_1);             
        ...
        //////////////////////////// SAVE GENERATION CHAMPION FILES /////////////////////////////////
        ...
        hcf_filename << "HyperNeat_conf_files/Champion_G" << g << "P" << generationChampionPopulation << ".txt";
        
        cppn_neat->organisms.at(generationChampionPopulation).save((char *)generation_champion_filename.str().c_str());
        hyperneat->printConnectionFile((char*)generation_champion_filename.str().c_str(), (char*)hcf_filename.str().c_str());
        ...
        {
            cout << "Error in cppn_neat->epoch(): " << endl;
        }
        ...
        for (int i=0;i < 8; i++){
            ...
            if((i+1)%3 == 0)
            {
                ...
                position.push_back(y_1[i]);
                position.push_back(0.05);
                vrep->setObjectPosition(obstacle, position);        //vrep->setObjectPosition(cubes.at(i), position);
                cubes.push_back(obstacle);
            }
    ...
    //outfile.close();
    //////////////////////////// SAVE CHAMPION OF ALL TIMES FILES /////////////////////////////////
    ...
    hyperneat->printConnectionFile((char*)"NEAT_organisms/Champion.txt",(char*)"HyperNeat_conf_files/Champion.txt");
